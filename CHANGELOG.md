...
- **New optional `serde` feature** (non-default) for serializing frames, errors, and interface configuration
    ...
    - `CanErrors` serializes as a flat array rather than exposing its internal head+tail layout, and deserializing an empty array is an **error** — the non-empty invariant holds across serde
    - `IdFlags`/`FdFlags` serialize as flag-name strings (`"BRS | FDF"`) via `bitflags`' serde support
    - `Error::Can` round-trips exactly. `Error::Io` is **lossy** by necessity, since `io::Error` implements neither serde trait: it is reduced to an `ErrorKind` name plus the message. After a round trip `raw_os_error()` is `None`, any `source()` chain is gone, and a kind with no stable name (or one a newer version introduced) arrives as `ErrorKind::Other`
...
- **Breaking:** an error frame now decodes into *all* the error conditions it describes, not just one. A single SocketCAN error frame routinely reports several conditions at once, and the old decoder discarded almost all of them
    ...
    - `CanErrorFrame::into_error()` renamed to `into_errors()`, returning `CanErrors`
    - `From<CanErrorFrame> for CanError` is replaced by `From<CanErrorFrame> for CanErrors`
    - Previously the decoder matched the *entire* error-class word against one value, so any frame setting more than one class bit fell through to `CanError::Unknown(bits)` and the whole error was lost. For example an `mcp251xfd` bus-error frame (`CAN_ERR_PROT | CAN_ERR_BUSERROR | CAN_ERR_ACK`) decoded to `Unknown(0xA8)`; it now decodes to seven distinct errors
    - `data[1]` (controller problems) and `data[2]` (violation types) are bitfields and are now decomposed bit by bit. The kernel's shared `can_change_state()` helper ORs both the TX and RX state codes into `data[1]` whenever the two states match, so `data[1] = 0x0C` is the normal encoding of a symmetric warning transition — it used to decode to `DecodingFailure`, and now yields two `ControllerProblem` errors
    - Entry order is now part of the contract: error classes ascending, and within a class byte, least-significant bit first
- **Breaking:** `CanError` gains `Counters { tx, rx }` for `CAN_ERR_CNT` frames, which were previously dropped entirely. Added `CAN_ERROR_WARNING_THRESHOLD`, `CAN_ERROR_PASSIVE_THRESHOLD` and `CAN_BUS_OFF_THRESHOLD` re-exports for interpreting the counter values
- **Breaking:** `CanError::TransceiverError` now carries a `TransceiverError` payload decoded from `data[4]`, which was previously never read at all — the whole `TransceiverError` enum was unreachable. Note `data[4]` is two independent nibbles (CAN High in the low half, CAN Low in the high half), so a fault on both lines decodes to two errors
- **Breaking:** `ControllerProblem::Active` renamed to `BackToErrorActive`. The kernel means "recovered *to* error-active state", which the old name read as the opposite
...
- `CanError::kind()` now maps protocol violations onto the specific `embedded_can::ErrorKind` values `Bit`, `Form` and `Stuff` rather than reporting everything as `Other`. `CanErrors::kind()` returns the first specific kind present, so a frame carrying both a controller warning and a missing ACK reports `Acknowledge`
- `CanErrors::Display` joins multiple errors with "; " and groups protocol violations that share a location, so a frame reporting five violation types at one location names the location once
- `CanError::Unknown(_)` is now printed in hex
- `ControllerProblem::ALL` and `ViolationType::ALL` expose the bit-valued variants for callers that need to walk the bitfields themselves
